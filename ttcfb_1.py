...
vang = '\033[1;33m'    # Vàng
xanh = '\033[1;36m'    # Xanh dương
tim = '\033[1;35m'     # Tím
trang = '\033[1;37m'   # Trắng sáng
thanh = '\033[1;34m'   # Xanh lam
...
{trang}        ║{do}    ╚═╝    ╚═════╝ ╚═╝  ╚═╝╚═╝  ╚═══╝             {trang}║
{trang}        ║                                                  ║
{trang}        ║       {do}⚡ TOOL BY TUAN TU ⚡ - SIÊU VIP 🔥        {trang}║
{trang}        ╚══════════════════════════════════════════════════╝
...
def clear_screen():
    print("\033c", end="")  # hoặc os.system('clear')

    print(Colorate.Horizontal(Colors.white_to_blue,"                © Bản Quyền tuấn tú  !!!!"))
    thanhngang(65)
    
def Delay(value):
    while not(value <= 1):
        value -= 0.123
        print(f'''{trang}[{xanh}TU{trang}] [{xanh}DELAY{trang}] [{xanh}{str(value)[0:5]}{trang}] [{vang}X    {trang}]''', '               ', end = '\r')
        sleep(0.02)
        print(f'''{trang}[{xanh}TU{trang}] [{xanh}DELAY{trang}] [{xanh}{str(value)[0:5]}{trang}] [ {vang}X   {trang}]''', '               ', end = '\r')
        sleep(0.02)
        print(f'''{trang}[{xanh}TU{trang}] [{xanh}DELAY{trang}] [{xanh}{str(value)[0:5]}{trang}] [  {vang}X  {trang}]''', '               ', end = '\r')
...
def addcookie():
    ...
    while True:
        i += 1
        cookie = input(f'{thanh}{luc}Nhập cookie fb{vang} {i}{trang}: {vang}')
        if cookie == '' and i != 1:
            break 
        fb = Facebook(cookie)
        info = fb.info()
        if 'success' in info:
            ...
            print(f'{do}Cookie Facebook Die , hoặc sai cookie !')
            i -= 1
...
if os.path.exists(f'cookiefb-ttc.json') == False:
    ...
    print(f'{thanh}{luc}Nhập {do}[{vang}1{do}] {luc}Sử Dụng Cookie Facebook Đã Lưu file')
...
print(f'{thanh}{luc}Nhập {do}[{vang}w{do}]{luc} Để Chạy Nhiệm Vụ Share Nội Dung')
...
for x in nhiemvu:
    list_nv.append(x)
list_nv = [x for x in list_nv if x in ['1','2','3','4','5','6','7','8','9','0', 'q', 'w']]
while(True):
    try:
...
while True:
    if len(listCookie) == 0:
        print(f'{do}Đã cookie chết, Vui Lòng Nhập Lại !!!')
        addcookie()
        with open('cookiefb-ttc.json','w') as f:
            json.dump(listCookie, f)
    for cookie in listCookie:
        ...
        if 'success' in info:
            ...
            print(f'{do}Cookie Facebook Die ! Đã Xóa Ra Khỏi file !!!')
        ...
        while True:
            ...
            if random_nv == 'w': fields = 'sharecheokemnoidung'
            chuyen = False
            try:
                ...
                if "idpost" in getjob.text or "idfb" in getjob.text:
                    ...
                    for x in getjob.json():
                        ...
                        if random_nv == "w": fb.sharend(x['idpost'].split('_')[1] if '_' in x['idpost'] else x['idpost'], json.loads(x["nd"])[0]); type = 'SHAREND'; id = x['idpost']; id_ = x['idpost'].split('_')[1] if '_' in x['idpost'] else x['idpost']
