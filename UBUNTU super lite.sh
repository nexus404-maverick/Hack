...
# Termux Lightweight Desktop (XFCE minimal + VS Code + Audio) - No Browser
set -euo pipefail
...
clear
echo -e "${GREEN}=== Cài desktop tối giản (XFCE + VS Code + Âm thanh) ===${NC}"
echo -e "${YELLOW}Không bao gồm trình duyệt web${NC}"
sleep 1
...
update_progress "Cài XFCE core + thunar + xterm"
...
update_progress "Cài PulseAudio (âm thanh) và VS Code"
install_pkg pulseaudio virglrenderer-android code-oss
...
# Khởi động virgl (tăng tốc đồ họa)
...
# Khởi động XFCE
xfce4-session &
echo "✓ Desktop đã khởi động (có âm thanh, không trình duyệt)"
...
# Tạo shortcut cho VS Code (hiển thị trong menu XFCE)
